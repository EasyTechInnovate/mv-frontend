...
// Reusable FormField component from HelpSupport.jsx
...
const MetaManualClaimForm = ({ onSubmit, isLoading, user }) => {
    const [fullName, setFullName] = useState(user?.fullName || '');
    const [email, setEmail] = useState(user?.emailAddress || '');
    const [mobile, setMobile] = useState(user?.mobileNumber || '');
    const [confirmation, setConfirmation] = useState(false);
    const [claims, setClaims] = useState([
        { metaVideoLink: '', officialVideoLink: '', isrc: '', startTime: '', endTime: '' }
    ]);
    ...
    const addClaim = () => {
        setClaims([...claims, { metaVideoLink: '', officialVideoLink: '', isrc: '', startTime: '', endTime: '' }]);
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {claims.map((claim, index) => (
                    <div key={index} className="p-4 border border-slate-700 rounded-lg space-y-4 relative">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                             <FormField label="Meta Video Link" required>
                                <Input
            ...
            <Button type="submit" disabled={isLoading} className="w-full bg-purple-600 text-white hover:bg-purple-700">
                {isLoading ? 'Submitting...' : 'Submit Meta Manual Claim'}
            </Button>
        </form>
    );
};

export default MetaManualClaimForm;