...
const YoutubeClaimReleaseForm = ({ onSubmit, isLoading, user }) => {
    const [fullName, setFullName] = useState(user?.fullName || '');
    const [email, setEmail] = useState(user?.emailAddress || '');
    const [mobile, setMobile] = useState(user?.mobileNumber || '');
    const [confirmation, setConfirmation] = useState(false);
    const [claims, setClaims] = useState([
        { youtubeVideoLink: '', officialVideoLink: '', isrc: '' }
    ]);
    ...
    const addClaim = () => {
        setClaims([...claims, { youtubeVideoLink: '', officialVideoLink: '', isrc: '' }]);
    };
    ...
    const handleSubmit = (event) => {
        ...
        for (const claim of claims) {
            if (!claim.youtubeVideoLink || !claim.isrc) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-start space-x-3">
                <input
                    type="checkbox"
                    id="confirm-youtube-claim"
                    className="rounded mt-1"
                    checked={confirmation}
                    onChange={(e) => setConfirmation(e.target.checked)}
                />
                <label htmlFor="confirm-youtube-claim" className="text-sm">
            ...
            <Button type="submit" disabled={isLoading} className="w-full bg-purple-600 text-white hover:bg-purple-700">
                {isLoading ? 'Submitting...' : 'Submit Youtube Claim'}
            </Button>
        </form>
    );
};

export default YoutubeClaimReleaseForm;
